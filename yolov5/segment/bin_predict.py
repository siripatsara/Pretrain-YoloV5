    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                # === ADDED: Save binary masks at thresholds 0.25 and 0.75 ===
                # if not isinstance(masks, torch.Tensor):
                #     masks = torch.from_numpy(masks)
                # for threshold in [0.25, 0.75]:
                #     binary_mask = torch.any(
                #         masks > threshold, dim=0).cpu().numpy().astype(np.uint8) * 255
                #     binary_mask_path = str(
                #         save_dir / f"{p.stem}_mask_{int(threshold*100)}.png")
                #     cv2.imwrite(binary_mask_path, binary_mask)

                # shape: (H, W), class index with max prob
                # composite_mask = masks.argmax(dim=0)
                # for threshold in [0.25, 0.75]:
                #     binary_mask = ((masks.max(dim=0).values) > threshold).cpu(
                #     ).numpy().astype(np.uint8) * 255
                #     binary_mask_path = str(
                #         save_dir / f"{p.stem}_mask_{int(threshold*100)}.png")
                #     cv2.imwrite(binary_mask_path, binary_mask)

                # Save binary masks at thresholds 0.25 and 0.75
                for threshold in [0.25, 0.75]:
                    for cls_idx in range(masks.shape[0]):
                        binary_mask = (masks[cls_idx] > threshold).cpu(
                        ).numpy().astype(np.uint8) * 255
                        binary_mask_path = str(
                            save_dir / f"{p.stem}_class{cls_idx}_mask_{int(threshold*100)}.png")
                        cv2.imwrite(binary_mask_path, binary_mask)
                ...
                annotator.masks(
                    ...
                    im_gpu=torch.as_tensor(im0, dtype=torch.float16).to(
                        device).permute(2, 0, 1).flip(0).contiguous()
                    / 255
                    if retina_masks
                    else im[i],
                )
...
def parse_opt():
    ...
    parser.add_argument("--weights", nargs="+", type=str,
                        default=ROOT / "yolov5s-seg.pt", help="model path(s)")
    ...
    parser.add_argument("--project", default=ROOT /
                        "runs/predict-seg", help="save results to project/name")
