    ...
    for epoch in range(start_epoch, epochs):
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            if RANK in {-1, 0}:
                ...
                if callbacks.stop_training:
                    return
                # Log losses to TensorBoard (per batch, optional)
                # writer.add_scalar("Batch/Loss/box", loss_items[0].item(), ni)
                # writer.add_scalar("Batch/Loss/objectness", loss_items[1].item(), ni)
                # writer.add_scalar("Batch/Loss/classification", loss_items[2].item(), ni)
            # end batch ------------------------------------------------------------------------------------------------
        ...
        if RANK in {-1, 0}:
            ...
            if writer is not None:
                writer.add_scalar("Loss/box", mloss[0].item(), epoch)
                writer.add_scalar("Loss/objectness", mloss[1].item(), epoch)
                writer.add_scalar("Loss/classification",
                                  mloss[2].item(), epoch)
                # If segmentation loss exists in results, log it
                if len(results) > 7:
                    writer.add_scalar("Loss/segmentation", results[7], epoch)
                writer.add_scalar("LearningRate", lr[0], epoch)
                writer.add_scalar("Metrics/Precision", results[0], epoch)
                writer.add_scalar("Metrics/Recall", results[1], epoch)
                writer.add_scalar("Metrics/mAP_0.5", results[2], epoch)
                writer.add_scalar("Metrics/mAP_0.5:0.95", results[3], epoch)
                # Accuracy plot (mAP@0.5)
                writer.add_scalar("Accuracy/mAP_0.5", results[2], epoch)
                # Precision-Recall curve (per class, if available)
                if maps is not None and hasattr(model, 'names'):
                    for cls_idx, cls_name in enumerate(model.names):
                        if cls_idx < len(maps):
                            writer.add_scalar(
                                f"PR/mAP_{cls_name}", maps[cls_idx], epoch)
